...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_SPI_1) || \
         ...
         
{
    ...
    (void) SPI_1_ReadTxStatus();

    /* Write TX data using blocking function */
    #if(0u != SPI_1_MODE_USE_ZERO)
        SPI_1_WriteTxDataZero(pData[buf_index]);
        buf_index++;
    #endif /* (SPI_1_MODE_USE_ZERO == 1u) */

    /* Write TX data using blocking function */
    do
    {
        SPI_1_WriteTxData(pData[buf_index]);
        buf_index++;
    }while(buf_index < size);

    while(0u != timeoutMs)
    {
        if(0u != (SPI_1_ReadTxStatus() & SPI_1_STS_SPI_DONE))
        {
            /* Return success code */
            *count = size;
         ...
         
{
        ...
        */
        if(0u != SPI_1_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = SPI_1_GetRxBufferSize();
                CyDelay(SPI_1_BYTE2BYTE_TIME_OUT);
            }while(SPI_1_GetRxBufferSize() > oldDataCount);
            status = CYRET_SUCCESS;
            break;
        }
    ...
    *count = 0u;
    ...
    while(SPI_1_GetRxBufferSize() > 0u)
    {
        tempCount = SPI_1_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = SPI_1_ReadRxData();
                dataIndexCntr++;
            }
...
#endif /* if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_SPI_1) || \
                                          (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface)) */
